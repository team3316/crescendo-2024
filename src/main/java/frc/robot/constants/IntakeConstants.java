package frc.robot.constants;

public class IntakeConstants {
    public final static int intakeMotorID = 11;
    public static final int rollerID = 0;

    public final static double collectingPercentage = 1;
    public final static double ejectPercentage = -0.8;
    public final static double disabledPrecent = 0;
    public static final double rollerCollectingPercentage = 1;
    public static final double rollerEjectPercentage = -0.8;
    public static final double rollerOffPercentage = 0;
    public final static int sensor_port = 5;

}
